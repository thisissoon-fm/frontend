...
export class SearchEffects {
  @Effect()
  public loadSearchResults$: Observable<Action> = this.actions$.pipe(
    ...
    concatMap((value: [Action, SearchState]) => {
      ...
      return state.query && state.query.length >= 3
        ? this.spotifySearchSvc.search(state.query, state.type).pipe(
            map(res => new fromSearchActions.LoadSearchResultsSuccess(res)),
            catchError(err =>
              of(new fromSearchActions.LoadSearchResultsFail(err))
            )
          )
  ...
  @Effect()
  public loadSearchResultsNextPage$: Observable<Action> = this.actions$.pipe(
    ofType(fromSearchActions.LOAD_SEARCH_RESULTS_NEXT_PAGE),
    withLatestFrom(this.store$.select(getSearchState)),
    filter(
      (value: [Action, SearchState]) =>
        value[1].query && value[1].query.length > 2
    ),
    concatMap((value: [Action, SearchState]) => {
      const state = value[1];
      ...
      return this.spotifySearchSvc.search(state.query, state.type, params).pipe(
        map(res => new fromSearchActions.LoadSearchResultsNextPageSuccess(res)),
        catchError(err =>
          of(new fromSearchActions.LoadSearchResultsNextPageFail(err))
        )
      );
    })
  );
