...
describe('PausedInterceptor', () => {
  ...
  it('should NOT get paused data from header', async(() => {
    http.get<QueueItem>(`${environment.apiUrlPlayer}some-other-url`)
      .subscribe(res => expect(res.paused).toBeUndefined());

    const request = httpMock.expectOne(`${environment.apiUrlPlayer}some-other-url`);
    const headers = new HttpHeaders();
    request.flush(queueItem);
    httpMock.verify();
  }));

  it('should get paused data from header', async(() => {
    http.get<QueueItem>(`${environment.apiUrlPlayer}player/current`)
      .subscribe(res => expect(res.paused).toBeFalsy());

    const request = httpMock.expectOne(`${environment.apiUrlPlayer}player/current`);
